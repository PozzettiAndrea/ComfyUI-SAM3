...
if (!app.extensionManager?.renderMarkdownToHtml) {
  loadScript('web_async/marked.min.js').catch((e) => {
    console.error(e)
  })
  loadScript('web_async/purify.min.js').catch((e) => {
    console.error(e)
  })
}

const categories = ["SAM3"];
const nodeDescriptions = new Map();

function isHelpPopupEnabled() {
  return app.ui.settings.getSettingValue("SAM3.helpPopup") !== false;
}
...
app.registerExtension({
    ...
    if (!isHelpPopupEnabled()) return;
        ...
        if (nodeData?.category?.startsWith(category)) {
			...
			console.error("Error in registering SAM3.HelpPopup", error);
		}
	},
	nodeCreated(node) {
    if (!isHelpPopupEnabled()) return;
